...
class GCSPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if item_name == 'CentauroItem':
            #raw_item = ItemAdapter(item)
            raw_item = item
            ean = raw_item.get("ean")
            if not ean:
                self.missing_ean_field_message(url=raw_item['url'], spider=spider)
                DropItem()
            timestamp_fmt = (
            ...
            year = datetime.datetime.now().year
            day = datetime.datetime.now().isoformat().split("T")[0]
            filename = f"{year}/{day}/{self.bot_name}/{timestamp_fmt}_{spider.name}_{raw_item.get('spider_version', '')}_{item_name}_{raw_item.get('ean')}.json"
    ...
    def missing_ean_field_message(self, url: str, spider) -> None:
        embed = DiscordEmbed(
            title=f'BOT {self.bot_name}/{spider.name} 💀 Missing EAN notifier.')
        embed.set_thumbnail(url=self.thumbnail)
        embed.set_timestamp()
        # add fields to embed
...
class CentauroImagePipeline(ImagesPipeline):
    
    def get_media_requests(self, item, info):
        if 'image_urls' in item:
            for image_url in item['image_urls']:
                request = Request(url=image_url)
                request.meta['ean'] = item.get('ean')
                yield request
    ...
    def file_path(self, request, response=None, info=None, *, item=None):
        ean = request.meta['ean']
        image_name = request.url.split('/')[-1]
        fname = f"{ean}_{request.url.split('/')[-2]}_{image_name}"
        return fname
